"""
S3: Gross Profitability Factor  (independent trial #8)
=======================================================
Signal   : GP/A = Trailing-twelve-month Gross Profit / Total Assets
           GP  = Revenue − COGS (TTM = sum of 4 most recent quarterly filings ≤ t)
           Direct GrossProfit tag preferred; falls back to Revenue − CostOfRevenue.
           Annual filing used as TTM proxy when <4 quarterly periods available.
           All inputs gated on SEC FILING DATE ≤ t (not period-end date).

Look-ahead discipline:
  - Every fundamental input gated on filed ≤ t.  A fiscal quarter ending
    Sep 30 that is filed Nov 5 is NOT knowable until Nov 5.
  - TTM sums the 4 most recent filed quarterly periods (start→end ≈ 90 days),
    not the 4 most recent fiscal periods. Avoids using a filing we don't yet have.
  - Total Assets = most recent balance-sheet value where filed ≤ t.

Portfolio: Long top 20% GP/A, short bottom 20%. Min 20 names/leg.
           Beta-neutral: 60-day OLS vs EW universe return (adjClose layer).
           Equal-weight within each leg; leg scaling as in S-IVOL/S-MAX.
Rebalance: Quarterly. Hold: 1 quarter. 24 periods over 2015-2021.
In-sample: 2015-01-01 → 2021-01-01. No walk-forward.
Trial    : #8 — genuinely new signal. H1+E1-E4+S9+(IVOL/MAX family)+S3-GP.
           DSR threshold rises with N_TRIALS=8.

Coverage note: COGS is a discrete XBRL tag only for manufacturing-heavy sectors.
Service and tech companies may report GrossProfit directly without a Revenue/COGS
split.  We try GrossProfit first, then Revenue − COGS.  Stocks with neither are
DROPPED from the portfolio and reported in the coverage audit each period.  A
large drop fraction (>50% of universe) would indicate the signal is being computed
on a biased sub-sample — flagged in the output.
...
# ── Signal parameters (locked a priori) ──────────────────────────────────────
TTM_QUARTERS      = 4      # number of quarterly periods for TTM
QTR_MIN_DAYS      = 75     # minimum calendar days to classify as quarterly period
QTR_MAX_DAYS      = 115    # maximum calendar days to classify as quarterly period
ANN_MIN_DAYS      = 300    # annual period lower bound
ANN_MAX_DAYS      = 400    # annual period upper bound
...
# ── GP XBRL concept hierarchy ────────────────────────────────────────────────
# Tried in order; first match with data wins.
...
COGS_CONCEPTS = [
    "CostOfGoodsSold",
    "CostOfRevenue",
    "CostOfGoodsSoldAndServicesCost",
]
SHARES_CONCEPTS = [
...
GP_NEEDED = set(
    GROSS_PROFIT_CONCEPTS + REVENUE_CONCEPTS + COGS_CONCEPTS
    + ["Assets"] + SHARES_CONCEPTS
)
...
# ── DSR: 8 independent trials after adding S3-GP ─────────────────────────────
N_TRIALS = 8   # H1 + E1 + E2 + E3 + E4 + S9 + (IVOL/MAX family) + S3-GP
...
# =============================================================================
# SECTION 7: SEC GP facts cache
# =============================================================================
#
# The mgmt_pit sec_facts_trimmed.json contains only shares + ROIC-related
# concepts. For GP we need Revenue/COGS/GrossProfit/Assets, which requires
# loading the full per-CIK fact files. We build and cache a GP-specific
# trimmed copy at CACHE/gp_facts_trimmed.json to avoid re-parsing 3k JSON
# files on every run.
...
def preload_gp_facts(ciks: list[int]) -> None:
    """
    Load GP XBRL concepts from trimmed cache, falling back to full per-CIK
    JSON files on first run and saving a new trimmed JSON for future runs.
    """
    t0 = time.time()

    if _GP_FACTS_PKL.exists():
        with open(_GP_FACTS_PKL) as f:
            raw = json.load(f)
        for k, v in raw.items():
            _GP_FACTS_CACHE[int(k)] = v
        # Pick up any CIKs added since the cache was built
        missing = [c for c in ciks if c not in _GP_FACTS_CACHE]
        if missing:
            _load_gp_facts_from_files(missing, save=False)
        loaded = sum(1 for v in _GP_FACTS_CACHE.values() if v is not None)
        print(f"[GPFacts] {loaded:,}/{len(ciks):,} loaded in {time.time()-t0:.1f}s")
        return

    print(f"[GPFacts] First run — loading {len(ciks):,} full fact files "
          f"(this takes ~1 min)...")
    _load_gp_facts_from_files(ciks, save=True)
    loaded = sum(1 for v in _GP_FACTS_CACHE.values() if v is not None)
    print(f"[GPFacts] {loaded:,}/{len(ciks):,} loaded in {time.time()-t0:.1f}s")


def _load_gp_facts_from_files(ciks: list[int], save: bool = True) -> None:
    for cik in ciks:
        p = CACHE / "sec_facts" / f"{cik}.json"
        if not p.exists():
            _GP_FACTS_CACHE[cik] = None
            continue
        try:
            with open(p) as f:
                full = json.load(f)
            ...
            for concept in GP_NEEDED:
                if concept in ug:
                    trimmed["facts"]["us-gaap"][concept] = ug[concept]
            _GP_FACTS_CACHE[cik] = trimmed
        except Exception:
            _GP_FACTS_CACHE[cik] = None
    if save:
        print("[GPFacts] Saving GP facts trimmed cache...")
        serializable = {str(k): v for k, v in _GP_FACTS_CACHE.items()}
        with open(_GP_FACTS_PKL, "w") as f:
...
# =============================================================================
# SECTION 8: Universe builder (identical to S-IVOL / S-MAX)
# =============================================================================
...
def build_universe(
    ...
    Shares-outstanding sourced from _GP_FACTS_CACHE (already loaded).
    """
...
# =============================================================================
# SECTION 9: GP signal computation (purely fundamental — no price layer)
# =============================================================================

def _period_days(start_str: str, end_str: str) -> int:
    """Calendar days between two YYYY-MM-DD strings. Uses proper date arithmetic."""
    try:
...
def _get_ttm_flow(
    ...
    """
    TTM value for a flow concept (Revenue, COGS, or GrossProfit).

    Strategy:
      1. Find all entries filed ≤ as_of with quarterly period (75–115 days).
      2. Deduplicate by period end-date, keeping most recently filed version.
      3. Sort by end-date descending; pick first 4 non-overlapping quarters.
      4. Sum → TTM.  Requires exactly 4 quarters; partial sums are unreliable.
      5. If <4 quarters: fall back to most recent annual period (300–400 days).
         Annual used as TTM proxy — always 12 months, but up to ~1yr stale.
    ...
    for concept in concept_list:
        ...
        # ── Try quarterly TTM ────────────────────────────────────────────
        if quarterly:
            # Deduplicate by end_date, keeping most-recently-filed revision
            by_end: dict = {}
            for filed, end, start, val in quarterly:
                if end not in by_end or filed > by_end[end][0]:
                    by_end[end] = (filed, end, start, val)

            # Sort by end_date descending; greedily pick non-overlapping quarters
...
def _get_assets_pit(facts: Optional[dict], as_of_str: str) -> Optional[float]:
    ...
    No period-length filter — balance sheet values are point-in-time.
    """
    if facts is None:
        return None
    try:
...
def compute_gp_signals(
    universe_df: pd.DataFrame,
    rebalance_date: pd.Timestamp,
) -> tuple:
    """
    Compute GP/A for each universe member.

    Look-ahead discipline:
      Every call to _get_ttm_flow / _get_assets_pit is gated on
      filed ≤ as_of_str (= rebalance_date in YYYY-MM-DD format).
      The 'quarterly' TTM method sums the 4 most recently FILED
      quarterly periods — not the 4 most recent fiscal periods.
      A quarter filed after t is excluded even if its fiscal period
      ended before t.

    Signal: gp_signal = GP/A (higher = more profitable = long)

    Coverage returns:
      n_signal   : names with valid gp_signal
      n_gp_direct: names where GrossProfit tag found directly
      n_gp_calc  : names where GP = Revenue − COGS
      n_annual_fb: names using annual fallback (not TTM quarters)
      n_no_cogs  : names with Revenue but no COGS (and no GrossProfit)
      n_no_data  : names with neither GrossProfit nor Revenue
      n_no_assets: names with GP but no Total Assets
    """
    as_of_str = rebalance_date.strftime("%Y-%m-%d")
    records   = []
    ...
    for _, row in universe_df.iterrows():
        ...
        # ── Step 1: TTM Gross Profit ───────────────────────────────────────
        # Try direct GrossProfit tag
        ...
        if gp_ttm is not None:
            ...
            if rev is not None and cgs is not None:
                gp_ttm     = rev - cgs
                gp_method  = f"rev_minus_cogs({rev_method})"
                n_gp_calc += 1
        ...
        records.append({
            "ticker":    row["ticker"],
            "gp_signal": gp_a,
            "gp_method": gp_method or "unknown",
        })

    signals_df = (
        pd.DataFrame(records)
        if records
        else pd.DataFrame(columns=["ticker", "gp_signal", "gp_method"])
    )
    coverage = {
        "n_universe":    len(universe_df),
        "n_signal":      len(signals_df),
        "n_gp_direct":   n_gp_direct,
        "n_gp_calc":     n_gp_calc,
...
def build_portfolio(signals_df: pd.DataFrame, betas: dict) -> tuple:
    """
    Long top 20% GP/A (high profitability), short bottom 20% (low profitability).
    Min 20 per leg.  Equal-weight within each leg.
    Beta-neutral sizing: long_$ × β_L = short_$ × β_S, gross = 2×.
    """
    df = signals_df.dropna(subset=["gp_signal", "raw_return"]).copy()
    n  = len(df)
    if n < 40:
    ...
    ranked = df.sort_values("gp_signal", ascending=False)
    longs  = ranked.iloc[:leg]["ticker"].tolist()   # high GP/A = long
    shorts = ranked.iloc[-leg:]["ticker"].tolist()  # low  GP/A = short
...
def _leg_return_detail(
    ...
    """
    Quarterly cost model: borrow /4, bid-ask × actual turnover.
    ...
    for t in tickers:
        ...
        if is_short:
            borrow_cost += (_borrow(mc) / 4.0) * w    # quarterly borrow = annual / 4
...
def run_backtest(
    ...
    for i, t in enumerate(REBALANCE_DATES):
        ...
        # ── 2. GP signal (fundamental — no price layer) ───────────────────────
        signals_df, cov = compute_gp_signals(univ, t)
        n_dropped = cov["n_universe"] - cov["n_signal"]
        pct_drop  = cov["pct_dropped"]
        print(f"  GP: {cov['n_signal']}/{cov['n_universe']} with signal "
              f"({pct_drop:.0f}% dropped) | "
              f"direct={cov['n_gp_direct']} calc={cov['n_gp_calc']} "
              f"annualFB={cov['n_annual_fb']} noCOGS={cov['n_no_cogs']} "
              f"noData={cov['n_no_data']}")
        if pct_drop > 50:
            print(f"  *** WARNING: {pct_drop:.0f}% of universe dropped. "
                  f"Signal computed on biased sub-sample. ***")
        ...
        # ── 9. IC: Spearman(gp_signal, realized_return) ───────────────────────
        # IC > 0: high GP/A outperformed (hypothesis confirmed).
...
def print_results(results: dict) -> None:
    ...
    total_direct = int(cov["n_gp_direct"].sum())
    total_calc  = int(cov["n_gp_calc"].sum())
    ...
    print(f"  With valid GP signal:                            {total_sig:,} ({100*total_sig/max(total_univ,1):.1f}%)")
    print(f"    — via GrossProfit tag (direct):                {total_direct:,}")
    print(f"    — via Revenue − COGS (calculated):             {total_calc:,}")
    print(f"    — used annual period (TTM fallback):           {total_annfb:,}")
    print(f"  Dropped (no signal):                             {total_univ-total_sig:,} ({overall_drop:.1f}%)")
    print(f"    — Revenue found, COGS missing:                 {int(cov['n_no_cogs'].sum()):,}")
    print(f"    — Neither GrossProfit nor Revenue:             {int(cov['n_no_data'].sum()):,}")
    print(f"    — GP found, Assets missing:                    {int(cov['n_no_assets'].sum()):,}")
    if overall_drop > 50:
        print(f"\n  *** WARNING: {overall_drop:.0f}% overall drop rate. "
              f"Portfolio computed on a potentially biased sub-sample. ***")
    else:
        print(f"\n  Coverage acceptable ({100-overall_drop:.0f}% coverage average).")

    print(f"\n{sep}")
    print("S3: GROSS PROFITABILITY — IN-SAMPLE RESULTS  (independent trial #8)")
    print("Signal: GP/A = TTM Gross Profit / Total Assets  (filed ≤ t, no look-ahead)")
    print("Long top 20% GP/A  |  Short bottom 20% GP/A  |  Beta-neutral  |  Quarterly")
    print(sep)

    print(f"\n  *** INFORMATION COEFFICIENT (cost-independent signal quality) ***")
    print(f"  Mean IC (Spearman, high-GP/A vs ret)  : {mean_ic:>12.4f}")
    print(f"  IC t-stat                              : {ic_tstat:>12.3f}")
    print(f"  IC > 0 = high GP/A outperformed (hypothesis confirmed).")
    ...
    print(f"  {'Mean ex-ante β long leg':<46} {mean_bl:>13.3f}")
    print(f"  {'Mean ex-ante β short leg':<46} {mean_bs:>13.3f}")
    ...
    print(f"  {'Date':<11} {'Net':>7} {'Gross':>7} {'Cost':>6} {'TOver':>6} "
          f"{'LScl':>5} {'SScl':>5} {'Mkt':>7} {'IC':>6} {'nL':>3} {'Drop%':>5}")
...
def save_results(results: dict) -> None:
    ...
    if not records:
        return
    pd.DataFrame(records).to_csv(RESULTS / "gp_period_returns.csv", index=False)
    pd.DataFrame(audit).to_csv(RESULTS / "gp_universe_audit.csv", index=False)
    pd.DataFrame(coverage).to_csv(RESULTS / "gp_coverage_audit.csv", index=False)
...
def log_registry(results: dict) -> None:
    ...
    row = {
        ...
        "study":                "gp_S3-GP",
        "hypothesis":           (
            "Profitable small/mid-cap firms (high gross profit-to-assets) "
            "outperform unprofitable ones. Effect under-arbitraged in low-coverage "
            "small caps. Novy-Marx (2013): quality vs value. "
            "Long top 20% GP/A, short bottom 20%. Beta-neutral."
        ),
        "data_source":          "Tiingo (raw close for returns) + SEC XBRL (GP/A signal, PIT)",
        "status":               "completed_in_sample",
        "trial_number":         8,
        "trial_note":           "Independent trial #8. Not a family member of any prior trial.",
        "n_trials_dsr":         N_TRIALS,
        "rebalance_freq":       "quarterly",
        "portfolio_note":       "Beta-neutral (EW market proxy, 60-day adjClose)",
        "signal_note":          "GP/A = TTM GrossProfit / Assets, filed ≤ t",
...
def main() -> None:
    print("=" * 78)
    print("S3: GROSS PROFITABILITY  (independent trial #8)")
    print("Signal: GP/A = TTM GrossProfit / Total Assets  (SEC XBRL, PIT gated)")
    print("Long top 20% GP/A  |  Short bottom 20% GP/A  |  Beta-neutral")
    print("Quarterly rebalance  |  In-sample: 2015-01-01 → 2021-01-01  |  24 periods")
    print(f"DSR: N_TRIALS={N_TRIALS}  |  threshold rises with each new independent trial")
    print("=" * 78)
    ...
    # ── Step 4: GP facts (Revenue / COGS / GrossProfit / Assets + shares) ──────
